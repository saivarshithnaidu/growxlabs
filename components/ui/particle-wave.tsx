...
const ParticleWave: React.FC<ParticleWaveProps> = ({ className = '' }) => {
  ...
  const initScene = () => {
    ...
    renderer.setClearColor(getBackgroundColor(currentTheme), 0); // alpha 0 for transparent container if needed
  ...
  const animate = () => {
    ...
    // renderer.setClearColor(getBackgroundColor(currentTheme)); // Managed by GlobalBackground
